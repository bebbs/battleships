...
describe Board do
  ...
  context 'a grid when initialised' do
  ...
  context 'placing ships methods' do
    ...
    it 'knows that the coordinates aren\'t clear, if there are ships' do
      board.grid[:A1], board.grid[:A2], board.grid[:A3] = cellA1, cellA2, cellA3
      allow(cellA1).to receive(:ship_or_water).and_return(:water)
  ...
  context 'placing a whole ship' do
    ...
    it 'places a ship in the cells when passed ship, start cell and orientation' do
